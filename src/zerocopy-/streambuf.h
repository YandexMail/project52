  ...
  , boost::noncopyable
{
  ...
  put_active_; // active 'put' fragment - this is where
...
public:
  ...
  segment_type detach (iterator const& split_point)
  {
    ...
    segment_type tmp (boost::make_iterator_range (get_active(), ++iter),
		      this->gptr(), split_point.cur_val_, get_allocator ());
  ...
  // advance put pointer by n bytes
...
#if defined(YDEBUG)
    std::cerr << "COMMIT_4, putsize=" << put_size() << ", tail_size=" << tail_size_
	      << ", buffer_size=" << (*put_active())->size() << "\n";
#endif
    inter_size_ += n;
...
protected:
  ...
  void promote_put()
  {
    ...
    // make sure there is at least one free fragment
    // after put_active
    reserve (1);
    // should always be zero
    // tail_size_ -= ((*put_active ())->end () - this->epptr ());
